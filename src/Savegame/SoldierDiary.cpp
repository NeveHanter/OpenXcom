...
bool SoldierDiary::manageCommendations(Mod *mod, std::vector<MissionStatistics*> *missionStatistics)
{
	...
	for (auto iter = commendationsList.begin(); iter != commendationsList.end(); )
	{
		...
		for (const auto& critPair : *commRule->getCriteria())
		{
			...
			const int giveMeAGoodName = critDef.at(nextCommendationLevel["noNoun"]);

			// Skip this medal if we have reached its max award level.
			if ((unsigned int)nextCommendationLevel["noNoun"] >= critDef.size())
			{
			...
			(
				...
				(
					(critName == "totalKills" && getKillTotal() < giveMeAGoodName) ||
					(critName == "totalMissions" && getMissionTotalFiltered(missionStatistics, commRule) < giveMeAGoodName) ||
					(critName == "totalWins" && getWinTotal(missionStatistics) < giveMeAGoodName) ||
					(critName == "totalScore" && getScoreTotal(missionStatistics) < giveMeAGoodName) ||
					(critName == "totalStuns" && getStunTotal() < giveMeAGoodName) ||
					(critName == "totalDaysWounded" && _daysWoundedTotal < giveMeAGoodName) ||
					(critName == "totalBaseDefenseMissions" && getBaseDefenseMissionTotal(missionStatistics) < giveMeAGoodName) ||
					(critName == "totalTerrorMissions" && getTerrorMissionTotal(missionStatistics) < giveMeAGoodName) ||
					(critName == "totalNightMissions" && getNightMissionTotal(missionStatistics, mod) < giveMeAGoodName) ||
					(critName == "totalNightTerrorMissions" && getNightTerrorMissionTotal(missionStatistics, mod) < giveMeAGoodName) ||
					(critName == "totalMonthlyService" && _monthsService < giveMeAGoodName) ||
					(critName == "totalFellUnconcious" && _unconciousTotal < giveMeAGoodName) ||
					(critName == "totalShotAt10Times" && _shotAtCounter10in1Mission < giveMeAGoodName) ||
					(critName == "totalHit5Times" && _hitCounter5in1Mission < giveMeAGoodName) ||
					(critName == "totalFriendlyFired" && (_totalShotByFriendlyCounter < giveMeAGoodName || _KIA || _MIA)) ||
					(critName == "total_lone_survivor" && _loneSurvivorTotal < giveMeAGoodName) ||
					(critName == "totalIronMan" && _ironManTotal < giveMeAGoodName) ||
					(critName == "totalImportantMissions" && getImportantMissionTotal(missionStatistics) < giveMeAGoodName) ||
					(critName == "totalLongDistanceHits" && _longDistanceHitCounterTotal < giveMeAGoodName) ||
					(critName == "totalLowAccuracyHits" && _lowAccuracyHitCounterTotal < giveMeAGoodName) ||
					(critName == "totalReactionFire" && getReactionFireKillTotal(mod) < giveMeAGoodName) ||
					(critName == "totalTimesWounded" && _timesWoundedTotal < giveMeAGoodName) ||
					(critName == "totalDaysWounded" && _daysWoundedTotal < giveMeAGoodName) ||
					(critName == "totalValientCrux" && getValiantCruxTotal(missionStatistics) < giveMeAGoodName) ||
					(critName == "isDead" && _KIA < giveMeAGoodName) ||
					(critName == "totalTrapKills" && getTrapKillTotal(mod) < giveMeAGoodName) ||
					(critName == "totalAlienBaseAssaults" && getAlienBaseAssaultTotal(missionStatistics) < giveMeAGoodName) ||
					(critName == "totalAllAliensKilled" && _allAliensKilledTotal < giveMeAGoodName) ||
					(critName == "totalAllAliensStunned" && _allAliensStunnedTotal < giveMeAGoodName) ||
					(critName == "totalWoundsHealed" && _woundsHealedTotal < giveMeAGoodName) ||
					(critName == "totalAllUFOs" && _allUFOs < giveMeAGoodName) ||
					(critName == "totalAllMissionTypes" && _allMissionTypes < giveMeAGoodName) ||
					(critName == "totalStatGain" && _statGainTotal < giveMeAGoodName) ||
					(critName == "totalRevives" && _revivedUnitTotal < giveMeAGoodName) ||
					(critName == "totalSoldierRevives" && _revivedSoldierTotal < giveMeAGoodName) ||
					(critName == "totalHostileRevives" && _revivedHostileTotal < giveMeAGoodName) ||
					(critName == "totalNeutralRevives" && _revivedNeutralTotal < giveMeAGoodName) ||
					(critName == "totalWholeMedikit" && _wholeMedikitTotal < giveMeAGoodName) ||
					(critName == "totalBraveryGain" && _braveryGainTotal < giveMeAGoodName) ||
					(critName == "bestOfRank" && _bestOfRank < giveMeAGoodName) ||
					(critName == "bestSoldier" && _bestSoldier < giveMeAGoodName) ||
					(critName == "isMIA" && _MIA < giveMeAGoodName) ||
					(critName == "totalMartyrKills" && _martyrKillsTotal < giveMeAGoodName) ||
					(critName == "totalPostMortemKills" && _postMortemKills < giveMeAGoodName) ||
					(critName == "globeTrotter" && (int)_globeTrotter < giveMeAGoodName) ||
					(critName == "totalSlaveKills" && _slaveKillsTotal < giveMeAGoodName)
				)
			)
			{
				awardCommendationBool = false;
